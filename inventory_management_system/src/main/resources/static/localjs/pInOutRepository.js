...
$('input[name="quantity"]').change(function () {
    ...
    $.ajax({
        type: "POST",
        url: "updateSelectProductQuantity",
        data: {
...
var productLength = $(".s1").attr("productLength");
$(".deleteProduct").on("click", function () {
    var obj = $(this);
    if (confirm("你确定要移除此商品吗？")) {
        $.ajax({
            type: "POST",
            url: "deleteSelectProduct",
            data: {
...
$("#submit").click(function () {
    ...
    //判断是否添加了物料，设置了物料的数量
    if (productLength != 0 && flag) $("#commit").submit();
    else alert("商品未添加或物料数量无填写!");
});
